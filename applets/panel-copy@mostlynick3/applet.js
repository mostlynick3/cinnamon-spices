...
MyApplet.prototype = {
    ...
    _init: function(metadata, orientation, panel_height, instance_id) {
        ...
        this._debugAppletManager();
        
        this._cleanOrphanedApplets();
        this._buildMenu();
    },
    
    _debugAppletManager: function() {
        try {
            const AppletManager = imports.ui.appletManager;
            
            global.log("Panel Copy DEBUG: Checking AppletManager...");
            global.log("Panel Copy DEBUG: appletMeta exists: " + (AppletManager.appletMeta !== undefined));
            
            if (AppletManager.appletMeta) {
                let keys = Object.keys(AppletManager.appletMeta);
                global.log("Panel Copy DEBUG: Number of applets in meta: " + keys.length);
                global.log("Panel Copy DEBUG: First 5 UUIDs: " + keys.slice(0, 5).join(", "));
                
                // Check if sound@cinnamon.org exists
                if (AppletManager.appletMeta['sound@cinnamon.org']) {
                    global.log("Panel Copy DEBUG: sound@cinnamon.org found!");
                    let meta = AppletManager.appletMeta['sound@cinnamon.org'];
                    global.log("Panel Copy DEBUG: sound metadata keys: " + Object.keys(meta).join(", "));
                    global.log("Panel Copy DEBUG: sound max-instances: " + meta['max-instances']);
                } else {
                    global.log("Panel Copy DEBUG: sound@cinnamon.org NOT found");
                }
            }
            
            // Also check appletDefinitions
            global.log("Panel Copy DEBUG: appletDefinitions exists: " + (AppletManager.appletDefinitions !== undefined));
            
        } catch (e) {
            global.log("Panel Copy DEBUG ERROR: " + e.toString());
        }
    },
...
_isMultipleInstanceApplet: function(uuid) {
    try {
        ...
        // Get the applet path from appletMeta
        if (!AppletManager.appletMeta || !AppletManager.appletMeta[uuid]) {
            global.log("Panel Copy: No applet path found for " + uuid + ", treating as single-instance");
            return false;
        }
        
        let appletPath = AppletManager.appletMeta[uuid].path;
        global.log("Panel Copy: Found path for " + uuid + ": " + appletPath);
        
        // Load the metadata.json file
        let metadataFile = Gio.File.new_for_path(appletPath + '/metadata.json');
        
        if (!metadataFile.query_exists(null)) {
            global.log("Panel Copy: metadata.json not found at " + appletPath + ", treating as single-instance");
            return false;
        }
        
        let [success, contents] = metadataFile.load_contents(null);
        if (!success) {
            global.log("Panel Copy: Could not read metadata.json for " + uuid + ", treating as single-instance");
            return false;
        }
        ...
        // If max-instances is not set, it defaults to single-instance
        if (!metadata.hasOwnProperty('max-instances')) {
            global.log("Panel Copy: " + uuid + " has no max-instances property (single-instance by default)");
            return false;
        }
        ...
        // -1 means unlimited instances, any number > 1 means multi-instance
        // 1 or missing means single-instance
        let isMulti = (maxInstances !== 1);
        global.log("Panel Copy: " + uuid + " max-instances=" + maxInstances + " (isMulti=" + isMulti + ")");
        return isMulti;
    ...
    _copyPanel: function(sourcePanelId, targetMonitor, targetPosition) {
        ...
        try {
            // Clean up orphaned applets first
            this._cleanOrphanedApplets();
            ...
            for (let i = 0; i < sourceApplets.length; i++) {
                ...
                // Format: panel1:left:0:menu@cinnamon.org:0
                let appletParts = applet.split(':');
                let zone = appletParts[1];
                let order = appletParts[2];
                
                // Get everything after the third colon
                let rest = appletParts.slice(3).join(':');
                
                // Find the LAST colon which separates the instance ID
                let lastColonIndex = rest.lastIndexOf(':');
                let fullUuid = rest.substring(0, lastColonIndex);  // This is "menu@cinnamon.org"
                let instanceId = rest.substring(lastColonIndex + 1);  // This is the instance number
                
                global.log("Panel Copy: Processing applet: " + fullUuid);
                
                if (!this._isMultipleInstanceApplet(fullUuid)) {
                    global.log("Panel Copy: Skipping single-instance applet: " + fullUuid);
                    skippedCount++;
                    continue;
                }
